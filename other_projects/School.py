...
try:
    ...
    subprocess.call("pip install sounddevice", shell=True)
    import sounddevice as sd
...
try:
    ...
    subprocess.call("pip install mkdir", shell=True)
    from mkdir import mkdir
...
try:
    ...
    subprocess.call("pip install git+https://github.com/MaliciousFiles/Windows-10-Toast-Notifications.git", shell=True)
    from win10toast import ToastNotifier
...
CHROMEDRIVER_PATH = os.path.abspath("bin\chromedriver.exe") if OPERATING_SYSTEM == WINDOWS else os.path.abspath("bin/chromedriver") if OPERATING_SYSTEM == MACOS else ""

if not os.path.exists(os.path.abspath("bin")):
    mkdir(os.path.abspath("bin"))
...
for courseIndex in range(len(courses)):
    ...
    try:
        ...
        for a in browser.find_elements_by_tag_name("a"):
            if "zoom" in a.get_attribute("href"):
                zooms.append(a)
        for link in zooms:
                ...
                if len(urls) > 1:
                    for tr in browser.find_elements_by_tag_name("tr"):
                        for td in tr.find_elements_by_tag_name("td"):
                            for p in td.find_elements_by_tag_name("p"):
                                if ("A" if aDay else "B") + str(courseIndex + 5 * (dayIndex)) in p.text:
                                    for i in range(5):
                                        parent = p
                                        for x in range(i):
                                        ...
                                        try:
                                            url = p.find_element_by_partial_link_text("zoom")
                                        except exceptions.NoSuchElementException:
                                            try:
                                                url = p.find_element_by_partial_link_text("Zoom")
                                            except exceptions.NoSuchElementException:
...
if not os.path.exists(os.path.abspath("Sounds/Alarm.wav")):
    subprocess.call("curl https://www42.online-convert.com/dl/web7/download-file/9607f7eb-1cf9-455e-b3a2-832f8ee03dee/Alarm.wav -o %s"%os.path.abspath("Sounds/Alarm.wav"))
        ...
        try:
            ...
            if hourMinute in times:
                ...
                if (zoomLink != "" or blendLink != "") and now.second == 0:
                    ...
                    if OPERATING_SYSTEM == WINDOWS:
                        toaster.show_toast(f"{activeClass} - {readableTimes[index]}",
                                           "Click to open the links", threaded=True,
                                           skip_queue=True, duration=15,
                                           sound_path=os.path.abspath("Sounds/Alarm.wav"),
                                           tooltip="School Reminders",
                                           callback_on_click=lambda:openClasses(blendLink, zoomLink))
                    elif OPERATING_SYSTEM == MACOS:
                        playsound(os.path.abspath("Sounds/Alarm.mp3"))
